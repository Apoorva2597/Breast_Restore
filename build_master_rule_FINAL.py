...
def pick_col(df: pd.DataFrame, options: List[str], required: bool = True) -> Optional[str]:
    for c in options:
        if c in df.columns:
            return c
    if required:
        raise RuntimeError(f"Required column missing. Tried={options}. Seen={list(df.columns)[:60]}")
    return None
...
def build_sectioned_note(note_text: str, note_type: str, note_id: str, note_date: str) -> SectionedNote:
...
def seed_master_from_structured() -> pd.DataFrame:
    struct_files = []
...
def load_and_reconstruct_notes() -> pd.DataFrame:
    note_files = []
    for g in NOTE_GLOBS:
        note_files.extend(glob(g, recursive=True))
    note_files = sorted(set(note_files))
    ...
    for fp in note_files:
        ...
        date_col = pick_col(df, ["NOTE_DATE_OF_SERVICE", "NOTE DATE OF SERVICE", "OPERATION_DATE",
                                 "ADMIT_DATE", "HOSP_ADMSN_TIME"], required=False)
        ...
        if line_col: keep_cols.append(line_col)
        if note_type_col: keep_cols.append(note_type_col)
...
def main():
    ...
    print(f"Master seeded: {len(master)} MRNs")

    print("Loading & reconstructing notes...")
    notes_df = load_and_reconstruct_notes()
    print(f"Reconstructed notes: {len(notes_df)}")
    ...
    print(f"Aggregated predictions for {len(best_by_mrn)} MRNs")
    ...
    print("\nDONE.")
    print(f"- Master (NO GOLD): {OUTPUT_MASTER}")
    print(f"- Evidence:         {OUTPUT_EVID}")
    print("\nRun:")
