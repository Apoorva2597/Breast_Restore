...
#
# Run examples:
#   python export_one_patient_deid_bundle.py --patient_id "63B0526207E98425D35E7EA737AB89AA"
...
DEID_INPUTS = [
    os.path.join(BASE_DIR, "DEID_FULLTEXT_HPI11526_Clinic_Notes_CTXWIPE_v3.csv"),  # clinic
    os.path.join(BASE_DIR, "DEID_FULLTEXT_HPI11526_NOTES_CTXWIPE_v4.csv"),         # op
    os.path.join(BASE_DIR, "DEID_FULLTEXT_HPI11526_NOTES_CTXWIPE_v5.csv"),         # inpatient
]

OUT_DIR = os.path.join(BASE_DIR, "PATIENT_BUNDLES")
...
def looks_like_ctxwipe_deid_filename(path):
    ...
    if "CTXWIPE" not in base:
        return False
    if "DEID" not in base and not base.startswith("DEID_"):
        return False
    return True
...
def read_deid_csv(path):
    return pd.read_csv(path, dtype=object, engine="python", encoding="utf-8", errors="replace")
...
def main():
    ...
    # Validate input files exist early (clear error)
    for p in DEID_INPUTS:
        if not os.path.exists(p):
            raise RuntimeError("Hard-coded input file not found: {}".format(p))
        if not looks_like_ctxwipe_deid_filename(p):
            raise RuntimeError("Refusing: does not look like DEID CTXWIPE file: {}".format(p))
    ...
    for path in DEID_INPUTS:
        ...
        if deid_col is None:
            raise RuntimeError("Could not detect a DE-ID text column in: {}".format(path))
        if "deid" not in deid_col.lower():
            raise RuntimeError("Refusing: detected text col doesn't look de-identified: {} in {}".format(deid_col, path))

        # Content-level check: confirm we see [NAME]/CTX_REDACTED markers
        found_marker = False
        ...
        if not found_marker:
            raise RuntimeError(
                "Refusing: could not find de-id markers in first {} rows of {}".format(scan_n, path)
            )
    ...
    if not all_rows:
        raise RuntimeError("No rows found across the hard-coded inputs.")
    ...
    patient_dir, timeline_path, combined_path = write_bundle(patient_id, patient_rows, OUT_DIR)
