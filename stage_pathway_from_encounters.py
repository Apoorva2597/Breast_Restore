...
#
# Purpose:
#   Build patient-level pathway staging using encounter files (not note text).
#
...
def min_dt_for_codes(enc, pid, codeset):
...
def min_dt_for_codes_after(enc, pid, codeset, min_dt=None):
    sub = enc[(enc["_PID_"] == pid) & (enc["CPT_CODE_STD"].isin(codeset))].copy()
    if len(sub) == 0:
        return pd.NaT
    sub = sub[sub["BEST_EVENT_DT_PARSED"].notnull()]
...
def main():
    ...
    for tag, path in enc_files:
        ...
        print("Loaded", len(df_small), "rows from", os.path.basename(path),
              "| pid_col:", pid_col, "| cpt_col:", cpt_col)
    ...
    for pid in sorted(cohort_pids):
        ...
        if len(sub) == 0:
            out_rows.append({
                ...
                "stage2_date": "",
                "stage2_def_cpt_set": "",
                "has_stage2_implant": False,
                "stage2_implant_date": "",
                "has_stage2_flap": False,
                "stage2_flap_date": "",
        ...
        out_rows.append({
            ...
            "counts_S2068": int((sub["CPT_CODE_STD"] == "S2068").sum()),
